...
function setRole(role) {
  selectedRole = role;
  if (roleTabAthlete)
    roleTabAthlete.classList.toggle("active", role === "athlete");
  if (roleTabExpert)
    roleTabExpert.classList.toggle("active", role === "expert");
  if (expertHint)
    expertHint.style.display = role === "expert" || isSignupMode ? "none" : "";
  if (loginCardTitle)
    loginCardTitle.textContent = isSignupMode
      ? role === "expert"
      ...
      : role === "expert"
        ...
        : "Welcome Back";
  if (loginCardSub)
    loginCardSub.textContent = isSignupMode
  ...
  if (emailInput)
    emailInput.placeholder =
      role === "expert" ? "Expert Email" : "Email or Mobile Number";
  if (loginBtnText)
    loginBtnText.textContent = isSignupMode
      ? role === "expert"
        ? "Create Expert Account"
        : "Create Account"
      : role === "expert"
...
if (roleTabAthlete)
  roleTabAthlete.addEventListener("click", () => setRole("athlete"));
if (roleTabExpert)
  roleTabExpert.addEventListener("click", () => setRole("expert"));
if (expertHintBtn)
  expertHintBtn.addEventListener("click", () => {
    setRole("expert");
  ...
  if (loginCardTitle)
    loginCardTitle.textContent = on
      ...
      : selectedRole === "expert"
        ...
        : "Welcome Back";
  if (loginCardSub)
    loginCardSub.textContent = on
      ? "Join ZITLAS - start your AI-powered fitness journey"
      : selectedRole === "expert"
        ? "Sign in to your ZITLAS Expert Portal"
        : "Sign in to continue your AI-powered fitness journey";
  if (loginBtnText)
    loginBtnText.textContent = on
      ? selectedRole === "expert"
        ? "Create Expert Account"
        : "Create Account"
      : selectedRole === "expert"
        ...
        : "Sign In";
  if (createLink)
    createLink.textContent = on
      ? "Already have an account? Sign In"
      : "Create Account";
  if (expertHint)
    expertHint.style.display = on || selectedRole === "expert" ? "none" : "";
}
...
if (typeof ZitlasAuth !== "undefined") {
  ZitlasAuth.onAuthStateChanged(async function (user) {
    if (!user) return;
    console.log(
      "[AUTH] onAuthStateChanged uid=" + user.uid + " email=" + user.email,
    );
    try {
      ...
      if (isExpert)
        console.log("[EXPERT FOUND] uid=" + user.uid + " email=" + user.email);

      try {
        await ZitlasDB.collection("users")
          .doc(user.uid)
          .update({
...
function showLoginOverlay() {
  ...
  if (msgEl) {
    msgEl.textContent =
      selectedRole === "expert" ? "Expert Portal Loading" : "Welcome back";
  }
...
if (emailInput)
  emailInput.addEventListener("input", () => clearInputError("emailGroup"));
if (passwordInput)
  passwordInput.addEventListener("input", () =>
    clearInputError("passwordGroup"),
  );
    ...
    try {
      if (isSignupMode) {
        const existingMethods =
          await ZitlasAuth.fetchSignInMethodsForEmail(email);
        ...
        if (selectedRole === "expert") {
          ...
          if (!existingExpert.empty) {
            console.log(
              "[EXPERT FOUND] by email before signup  aborting account creation",
            );
        ...
        const cred = await ZitlasAuth.createUserWithEmailAndPassword(
          email,
          password,
        );
        ...
        try {
          ...
          if (selectedRole === "expert") {
            console.log(
              "[PROFILE CREATED] new expert uid=" +
                user.uid +
                " email=" +
                user.email,
            );
            ...
            setLoading(false);
            showToast(
              "Expert account created! Your application is under review.",
            );
            setTimeout(
          ...
          console.warn(
            "[AUTH] Firestore setup failed  deleting orphan Auth account",
            firestoreErr,
          );
          try {
            await user.delete();
        ...
        if (rememberInput?.checked)
          localStorage.setItem("zitlas_remember", "true");
      }
    } catch (err) {
      setLoading(false);
...
// 1. CHECK FOR REDIRECT RESULT ON PAGE LOAD
// This handles the return from Google/Firebase Auth back to your web app
document.addEventListener("DOMContentLoaded", async () => {
  if (typeof ZitlasAuth === "undefined") return;

  try {
    const result = await ZitlasAuth.getRedirectResult();
    if (result && result.user) {
      console.log("[REDIRECT AUTH SUCCESS] User returned from Google Login");
      await handleGoogleUserSuccess(result.user);
    }
  } catch (err) {
    console.error("[ZITLAS] Redirect auth error:", err);
    showToast(err.message || "Sign-in failed. Please try again.");
  }
});

// 2. GOOGLE LOGIN BUTTON EVENT LISTENER
if (googleBtn) {
...
// 3. REUSABLE USER PROCESSOR
async function handleGoogleUserSuccess(user) {
  ...
  if (doc.exists) {
    ...
    console.log(
      "[USER FOUND] uid=" + user.uid + " resolvedRole=" + resolvedRole,
    );
    if (isExpert)
      console.log("[EXPERT FOUND] uid=" + user.uid + " email=" + user.email);
    ...
    console.log(
      "[GOOGLE LOGIN] No users doc for uid=" +
        user.uid +
        " checking experts by email",
    );
...
function syncFirebaseUser(user, role) {
  ...
  if (typeof ZitlasAccountGuard !== "undefined")
    ZitlasAccountGuard.beginSession(user.uid);
...
function showExpertApplicationReview(email) {
  ...
  document
    .querySelectorAll(".grm-sub, .grm-options, .grm-confirm-btn")
    .forEach(function (el) {
